...
void test_mutex_locked_queue(void) {
  const size_t capacity_for_tests = 3;
  AlogMutexLockedQueue queue =
      alog_mutex_locked_queue_create(capacity_for_tests, sizeof(int));

  bool test_res = false;
  int test_data = 0;

  for (size_t i = 0; i < capacity_for_tests; i++) {
    if (!(test_res = alog_mutex_locked_queue_push(&queue, &i)))
      continue;
  }
  test_condition("Can push until full", test_res);
  test_res = alog_mutex_locked_queue_push(&queue, &test_data);
  test_condition("Push on full returns false", test_res == false);

  for (size_t i = 0; i < capacity_for_tests; i++) {
    int data;
    if (!(test_res = alog_mutex_locked_queue_pop(&queue, &data)))
      continue;
  }
  test_condition("Can pop until empty", test_res);
  test_res = alog_mutex_locked_queue_pop(&queue, &test_data);
  ...
  alog_mutex_locked_queue_free(&queue);
